...
ua_realmi = 'Mozilla/5.0 (Linux; Android 11; RMX3191) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.40 Safari/537.36;]'
ua_sony = 'Mozilla/5.0 (Linux; U; Android 2.3.4; pt-pt; SonyEricssonLT18a Build/4.0.1.A.0.266) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1;]'
...
def banner():
    ...
    _logo_line_3_ = ('%s / _// / / __/ -_)   / /   ║  %sCoded By Sk hanz  %s║'%(O,P,O))
    _logo_line_4_ = ('%s/___/_/_/\__/\__/  _/ /_   ║    %s• ANONYMOUSE •    %s║'%(O,P,O))
...
def ugen_hp():
    ...
    _dapunta_cici_('%s╠══[%s9%s] %sRealmi'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s10%s] %sSony'%(O,P,O,P))
    ...
    elif pc in ['9','08']:ugent = _dapunta_dapunta_('ugent.txt','w');ugent.write(ua_realmi);ugent.close()
    elif pc in ['10','010']:ugent = _dapunta_dapunta_('ugent.txt','w');ugent.write(ua_sony);ugent.close()
    else:jalan('%s╚══[%s!%s]%sIsi Yang Benar'%(M,P,M,P));menu()
